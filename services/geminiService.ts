...
import { PersonalityProfile, Gender, ArtStyleKey } from "../types";
...
const VISUAL_MAP: Record<string, string> = {
  ...
  ENTD: "Theme: 'Love Player/Gamer'. Visuals: Cyberpunk-lite casual streetwear with neon accents. Holding a stylized futuristic game controller. Surrounded by pixel-art hearts and glitches. Confident, mischievous grin. Electric purple and cyan.",
...
/**
 * Generates an avatar with the specific "High Quality Anime" aesthetic.
 * Incorporates specific visual traits based on the user's personality nickname.
 */
export const generateAvatar = async (profile: PersonalityProfile, gender: Gender, styleKey: ArtStyleKey = 'anime'): Promise<string> => {
  ...
  const stylePrompt = ART_STYLES[styleKey].prompt;
  ...
  **VISUAL SPECIFICATIONS (Strict):**
  ...
  - **Pose:** ${randomPose}.
  - **Background:** Simple graphic background with elements matching the character's theme (e.g., pixel hearts, floating shapes, gradients). High quality illustration.
